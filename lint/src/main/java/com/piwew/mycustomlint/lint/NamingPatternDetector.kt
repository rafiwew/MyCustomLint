...
class NamingPatternDetector : Detector(), Detector.UastScanner {
            ...
            override fun visitClass(node: UClass) {
                if (node.name?.isDefinedCamelCase() == false) {
                    context.report(
                        ...
                        "Penulisan nama class harus menggunakan gaya penulisan CamelCase."
                    )
                }
            }
        }
    ...
    companion object {
        val ISSUE_NAMING_PATTERN: Issue = Issue.create(
            id = "NamingPattern",
            briefDescription = "Penulisan nama class harus menggunakan gaya penulisan CamelCase.",
            explanation = """
                Tulis nama class dengan menggunakan CamelCase.
                Lihat contoh yang benar pada situs ini https://google.github.io/styleguide/javaguide.html#s5.3-camel-case
            """.trimIndent(),
